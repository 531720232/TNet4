...
namespace TNet.Serialization
{
    ...
    public static class ProtoBufUtils
    {
        ...
        {
            return SerializeAuto7Zip(obj, autoUseGzip);
        }
        ...
        {
            return DeserializeAuto7Zip(data, type, isGzip);
        }
        ...
        /// <param name="autoUseGzip"> </param>
        /// <returns></returns>
        internal static Byte[] SerializeAuto7Zip(object obj, bool autoUse7zip = true)
        {
        ...
        {
            if (data.Length >= 14 && data[0] == 93 && data[1] == 0 && data[2] == 0 && data[3] == 64 && data[4] == 0 
                       )
            {
                return true;
            }
        ...
        /// <summary>
        /// 反序列化二进制数组为对象,并7zip压缩
        /// </summary>
        /// <param name="data"></param>
        /// <param name="type"></param>
        /// <param name="isGzip"></param>
        /// <returns></returns>
        internal static object DeserializeAuto7Zip(Byte[] data, Type type, bool is7zip)
        {
            try
            {
                if (!is7zip)
                ...
                var buff = LZMA.Decompress(data);

                   
                    return typeModel.Deserialize(new MemoryStream(buff), null, type);
        ...
        public static void LoadProtobufType(Assembly assembly)
        {
            ...
            for (int i = 0; i < types.Count; i++)
            {
                ...
                {
                    GameLoger.WriteError("Loading protobuf type \"{0}\" error:{1}", myEntity.FullName, e);
                }
        ...
        private static void LoadProtoTypeMember(Type myEntity, List<PropertyInfo> Properties, MetaType metaType, List<FieldInfo> Fields, out int maxMemberTag)
        {
            ...
            Properties.ForEach((o) =>
            {
                ...
                {
                    GameLoger.WriteError("Loading protobuf type \"{0}.{1}\" property error:{2}", myEntity.FullName, o.Name, ex);
                }
            });
            Fields.ForEach((o) =>
            {
                ...
                {
                    GameLoger.WriteError("Loading protobuf type \"{0}.{1}\" field error:{2}", myEntity.FullName, o.Name, ex);
                }
            });

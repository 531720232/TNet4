    ...
    public class MessageHandler : BaseMessageProcessor
    {
        ...
        public override bool TryReadMeaage(DataToken dataToken, byte[] buffer, out List<DataMessage> messageList)
        {
            ...
            do
            {
                if (dataToken.prefixBytesDone < 4)
                {
                    ...
                    if (dataToken.prefixBytesDone == 4 && (dataToken.messageLength > 10 * 1024 * 1024 || dataToken.messageLength <= 0))
                    {
                        ...
                        TraceLog.Write("The host[{0}] request parser head byte error, byte len={1}\r\ndata:{2}",
                            dataToken.Socket.RemoteEndPoint.ToString(),
                ...
                if (dataToken.IsMessageReady)
                {
                    ...
                    messageList.Add(new DataMessage() { Data = dataToken.byteArrayForMessage, OpCode = OpCode.Binary });
        ...
        private int HandleMessage(byte[] buffer, int offset, DataToken dataToken, int remainingBytesToProcess)
        {
            ...
            if (remainingBytesToProcess + dataToken.messageBytesDone >= dataToken.messageLength)
            {
                ...
                System.Buffer.BlockCopy(buffer, offset, dataToken.byteArrayForMessage, dataToken.messageBytesDone, copyedBytes);
                dataToken.messageBytesDone = dataToken.messageLength;
                dataToken.bufferSkip += copyedBytes;
            }
            else
            {
                System.Buffer.BlockCopy(buffer, offset, dataToken.byteArrayForMessage, dataToken.messageBytesDone, remainingBytesToProcess);
        ...
        public override byte[] BuildMessagePack(TNSocket exSocket, sbyte opCode, byte[] data, int offset, int count)
        {
            ...
            System.Buffer.BlockCopy(BitConverter.GetBytes(count), 0, buffer, 0, 4);
            System.Buffer.BlockCopy(data, offset, buffer, 4, count);

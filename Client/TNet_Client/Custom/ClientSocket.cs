...
namespace TNet.RPC.Sockets
{
    ...
    public class ClientSocket : ISocket, IDisposable
    {
        ...
        public AutoResetEvent receiveWaitEvent = new AutoResetEvent(false);
        /// <summary>
        /// 
        /// </summary>
        ...
        public void Bind(Socket acceptSocket)
        {
        ...
        /// <param name="offset"></param>
        /// <param name="count"></param>
        public override  void PostSend(TNSocket socket, byte[] data, int offset, int count)
        {
        ...
        /// <param name="offset"></param>
        /// <param name="count"></param>
        public override void  PostSend(TNSocket socket, sbyte opCode, byte[] data, int offset, int count)
        {
        ...
        public override void  PostSend(TNSocket socket, sbyte opCode, byte[] data, int offset, int count, Action<SocketAsyncResult> callback)
        {
            byte[] buffer = requestHandler.MessageProcessor.BuildMessagePack(socket, opCode, data, offset, count);
             SendAsync(socket, buffer, callback);
        }
        ...
        /// </summary>
        /// <param name="socket"></param>
        public override void Ping(TNSocket socket)
        {
        ...
        /// </summary>
        /// <param name="socket"></param>
        public override void Pong(TNSocket socket)
        {
        ...
        /// <param name="callback"></param>
        internal protected override bool SendAsync(TNSocket socket, byte[] buffer, Action<SocketAsyncResult> callback)
        {
        ...
        private void PostSend(SocketAsyncEventArgs ioEventArgs)
        {
            ...
            if (dataToken.messageLength - dataToken.messageBytesDone <= this.clientSettings.BufferSize)
            {
                ...
                System.Buffer.BlockCopy(dataToken.byteArrayForMessage, dataToken.messageBytesDone, ioEventArgs.Buffer, dataToken.bufferOffset, dataToken.messageLength - dataToken.messageBytesDone);
            }
            else
            {
                this.sendEventArg.SetBuffer(dataToken.bufferOffset, this.clientSettings.BufferSize);
                System. Buffer.BlockCopy(dataToken.byteArrayForMessage, dataToken.messageBytesDone, ioEventArgs.Buffer, dataToken.bufferOffset, this.clientSettings.BufferSize);
            }
        ...
        /// <param name="opCode"></param>
        /// <param name="reason"></param>
        public void Dispose(TNSocket exSocket, sbyte opCode, string reason)
        {
            try
            {
                ...
                {
                    Socket = exSocket,
                    Source = new DataMessage() { OpCode = opCode, Message = reason }
                };
                DoClosed(e);
                OnDisconnected(e);
    ...
    {
        internal static void Write(string v,params object[] args)
        {
            string str = v;
            if (args.Length > 0)
            {
                str = string.Format("Log>>" + v, args);
            }
            TNet.Client.Log.GameLoger.Write(str);
        }

        internal static void WriteError(string v, params object[] args)
        {
            string str = v;
            if (args.Length > 0)
            {
                str = string.Format("Trace>>" + v, args);
            }
            TNet.Client.Log.GameLoger.WriteError(str);

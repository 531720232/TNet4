...
namespace TNet.RPC.IO
{
    ...
    public class MessageStructure : BaseDisposable
    {
        ...
        /// 启用7zip压缩的最小字节
        /// </summary>
        public static int Enable7zipMinByte { get; set; }

    
        /// <summary>
        /// 开启7zip压缩
        /// </summary>
        public static bool Enable7zip { get; set; }

        static MessageStructure()
        {
            Enable7zip = true;
            Enable7zipMinByte =  10240;
        ...
        /// <summary>
        /// Read and remove buffer of 7zip.
        /// </summary>
        /// <returns></returns>
        public byte[] Pos7zipBuffer()
        {
            var buffer = PopBuffer();
            if (Enable7zip && buffer.Length > Enable7zipMinByte)
            {
                buffer = Compress7zipBuffer(buffer);
            }
            return buffer;
        }
        /// <summary>
        ...
        /// <summary>
        /// 读取消息头，如果有7zip压缩时自动解压字节流
        /// </summary>
        /// <returns></returns>
        /// <exception cref="OverflowException"></exception>
        public MessageHead ReadHead7zip()
        {
            try
            {
                MessageHead head = new MessageHead();
                ...
                if (Check7zipBuffer())
                {
                    byte[] lzmaData = PopBuffer();
                    head._7zipLength = lzmaData.Length;
              
                    //启用7z?
                    byte[] deZipData = LZMA.Decompress(lzmaData, 0);// GzipUtils.DeCompress(gzipData, 0, gzipData.Length);
                    WriteByte(deZipData);
                    Reset();
                }
                head.TotalLength = ReadInt();
        ...
        /// </summary>
        /// <returns></returns>
        private bool Check7zipBuffer()
        {
            byte[] _7zipHead = PeekByte(IntSize);
            return CheckEnable7zip(_7zipHead);
        }
        ...
        /// <returns></returns>
        public bool CheckEnable7zip(byte[] data)
        {
            if (data != null && data.Length > 3)
            {
                return data[0] == 93 && data[1] == 0x00 && data[2] == 0x00 && data[3] ==64;
            }
            return false;
        }
        ...
        /// <param name="use7zip"></param>
        public void WriteByte(object obj, bool use7zip = true)
        {
            try
            {
                var bytes = ProtoBufUtils.Serialize(obj, use7zip);
                byte[] lengthBytes = BitConverter.GetBytes(bytes.Length);
                WriteByte(lengthBytes);
                if (bytes.Length > 0)
                {
        ...
        /// <param name="obj"></param>
        /// <param name="use7zip"></param>
        public void PushIntoStack(object obj, bool use7zip)
        {
            try
            {
                var bytes = ProtoBufUtils.Serialize(obj, use7zip);
        ...
        /// <summary>
        /// 写入启用7zip压缩的字节流，格式： Len(4) + 7zip(buffer)
        /// </summary>
        /// <param name="buffer">未压缩的字节流</param>
        public void Write7zipBuffer(byte[] buffer)
        {
            int orgStackSize = BitConverter.ToInt32(buffer, 0);
            if (Enable7zip && orgStackSize > Enable7zipMinByte)
            {
                Write7zipCompressBuffer(buffer);
            }
            else
            {
        ...
        public void WriteBuffer(MessageHead head)
        {
            ...
            bool has7zipHead = false;
            if (head.Has7zip && (!Enable7zip || (Enable7zip && stackSize <= Enable7zipMinByte)))
            {
                has7zipHead = true;
                head._7zipLength = stackSize;
                WriteByte(head._7zipLength);
            }
            ...
            WriteStackToBuffer();
            //判断是否有启用7zip
            if (head.Has7zip && Enable7zip && !has7zipHead && stackSize > Enable7zipMinByte)
            {
                byte[] orgData = PopBuffer();
                head._7zipLength = Write7zipCompressBuffer(orgData);
            }
            head.PacketLength = Length;
            Reset();
        }
        ...
        /// <summary>
        /// 写入7zip压缩的Buffer，返回压缩后的长度
        /// </summary>
        /// <returns></returns>
        private int Write7zipCompressBuffer(byte[] orgData)
        {
            byte[] buffer = Compress7zipBuffer(orgData);
            int lzmaLength = buffer.Length;
            //这里不需要再加Len长度，在socekt传送时会统一增加包的长度
            WriteByte(buffer);
            return lzmaLength;
        }

        private static byte[] Compress7zipBuffer(byte[] data)
        {
            return LZMA.Compress(data);//GzipUtils.EnCompress(data, 0, data.Length);
        }

...
namespace TNet.RPC.Sockets
{
    ...
    public class SocketListener : ISocket
    {
        ...
        /// 
        /// </summary>
        public event Action<TNSocket, int> OnClosedStatus;

        private void DoClosedStatus(TNSocket socket, int statusCode)
        {
            Action<TNSocket, int> handler = OnClosedStatus;
        ...
        /// </summary>
        /// <param name="socketSettings"></param>
        public SocketListener(SocketSettings socketSettings,bool ws)
            : this(socketSettings, new ws.WebSocketRequestHandler())
        {
        ...
        {
            listenSocket = new Socket(this.socketSettings.LocalEndPoint.AddressFamily, SocketType.Stream, ProtocolType.IP);
        ...
        private void ProcessAccept(SocketAsyncEventArgs acceptEventArgs)
        {
            try
            {
                ...
                else
                {
                    ...
                    var exSocket = new TNSocket(ioEventArgs.AcceptSocket);
        ...
        public override async Task PostSend(TNSocket socket, byte[] data, int offset, int count)
        {
        ...
        /// <param name="offset"></param>
        /// <param name="count"></param>
        public override async Task PostSend(TNSocket socket, sbyte opCode, byte[] data, int offset, int count)
        {
        ...
        public override async Task PostSend(TNSocket socket, sbyte opCode, byte[] data, int offset, int count, Action<SocketAsyncResult> callback)
        {
        ...
        /// <param name="callback"></param>
        internal protected override async Task<bool> SendAsync(TNSocket socket, byte[] buffer, Action<SocketAsyncResult> callback)
        {

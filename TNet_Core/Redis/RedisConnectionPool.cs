...
namespace TNet.Redis
{
    ...
    public static class RedisConnectionPool
    {
        ...
        private static void InitRedisInfo()
        {
            ...
            {
                var js = cli.GetValue(RedisInfoKey);

           //     var obj = Newtonsoft.Json.JsonConvert.DeserializeObject<Redis.RedisInfo>(cli.GetValue(RedisInfoKey));
                //redisInfo = new RedisInfo();
                //redisInfo.ClientVersion = obj["ClientVersion"].ToString().ToEnum<RedisStorageVersion>();
                //redisInfo.HashCode = obj["HashCode"].ToString();
                //redisInfo.ServerHost = obj["ServerHost"].ToString();
                //redisInfo.ServerPath = obj["ServerPath"].ToString();
                //redisInfo.SerializerType = obj["SerializerType"].ToString();
                //redisInfo.SlaveSet =obj["SlaveSet"].ToObject(typeof(Dictionary<string, RedisInfo>)) as Dictionary<string, RedisInfo>;
                redisInfo =JsonUtils.Deserialize<RedisInfo>(js) ?? new RedisInfo();
                ...
                return true;
            }, trans => trans.QueueCommand(c => {

               
              //  var jhs = MathUtils.ToJson(redisInfo).ToString();
                //jhs = jhs.TrimStart('\"');
                //jhs = jhs.TrimEnd('\"');
                //jhs = jhs.Replace("\\", "\\");
                c.Set(RedisInfoKey, redisInfo);
            }));
